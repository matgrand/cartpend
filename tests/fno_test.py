...
USE_LIBRARY = True  # True: neuralop FNO | False: from-scratch FNO
...
N_TRAJ  = 64 * 512
T_END   = 1.0
N_STEPS = 30      # trajectory has N_STEPS+1 time points
U_MAX   = 5.0
SEED    = 42
...
class SpectralConv1d(nn.Module):
    ...
    def forward(self, x):  # (B, C_in, T)
        B, _, T = x.shape
        x_ft   = torch.fft.rfft(x, norm="forward")             # (B, C_in, T//2+1)
        ...
        return torch.fft.irfft(out_ft, n=T, norm="forward")    # (B, C_out, T)
...
def train(x_data, y_data):
    ...
    for epoch in (bar := tqdm(range(1, N_EPOCHS+1), bar_format=BAR_FMT)):
        ...
        loss = F.mse_loss(pred, y_data[idx])
        opt.zero_grad(); loss.backward(); opt.step(); sched.step()
        losses.append(loss.item())
        if epoch % 10 == 0:
            bar.set_description(f"Epoch {epoch:4d} | Loss {torch.tensor(losses[-10:]).mean():.2e}")
    return model, losses
...
def plot_loss(losses):
    ...
    plt.semilogy(losses); plt.xlabel("Epoch"); plt.ylabel("MSE"); plt.title("Training loss")
    plt.tight_layout(); plt.savefig(LOSS_PATH, dpi=150, bbox_inches="tight"); print(f"Saved -> {LOSS_PATH}")
    if SHOW_PLOTS: plt.show()


def plot_eval(model, x_data, y_data, t, n_cols=3):
    model.eval()
    ...
    for col in range(n_cols):
        with torch.no_grad(): pred = model(x_data[col:col+1].to(device)).squeeze(0).cpu()
        true = y_data[col]
...
if __name__ == "__main__":
    ...
    x_data, y_data, t = make_dataset()
    print(f"Dataset: {x_data.shape[0]} trajectories, {x_data.shape[-1]} time steps")
    model, losses = train(x_data, y_data)
    plot_loss(losses)
    plot_eval(model, x_data.cpu(), y_data.cpu(), t)
